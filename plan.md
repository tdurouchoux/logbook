...
- [ ] `src/main.ts` — plugin entry point only: `onload`/`onunload`, settings load/save, view registration, ribbon icon/command
- [ ] `src/settings.ts` — `LogbookSettings`, defaults, `LogbookSettingTab`
- [ ] `src/types.ts` — `NoteTypeConfig`, `NOTE_TYPES`, per-type frontmatter interfaces/discriminated union, shared constants
- [ ] `src/note-store.ts` — vault/metadata-cache access: loading notes, the per-file `processFrontMatter` write queue, refresh-suppression window, draft auto-delete, create/rename helpers
- [ ] `src/view/LogbookView.ts` — the `ItemView` shell: layout, vault event wiring, scroll/refresh orchestration (delegates to the modules below rather than owning everything itself)
- [ ] `src/view/feed.ts` — feed rendering, day grouping, history pagination/sentinel, empty states
- [ ] `src/view/card.ts` — note card (collapsed + expanded), one render function per type-specific field block
- [ ] `src/view/pickers.ts` — the reusable chip+autocomplete picker (tags/projects/teams currently duplicate this logic once team/tag pickers are added — factor it out once, not three times)
- [ ] `src/view/dock.ts` — command bar: input/dropdown state machine, command parsing, filter-chip rendering
- [ ] `src/filters.ts` — filter state model and the predicate that applies it to a note list
- [ ] `src/utils.ts` — `groupByDay`, `sameDay`, `relativeTime`, slug generation, and other pure helpers
- [ ] Build check: `esbuild.config.mjs` already globs nothing special — confirm it still bundles correctly once `main.ts` is broken into multiple modules (no config change expected, just verify)
...
- [ ] Draft: confirm no extra fields; wire into auto-delete (Phase 10)
...
- [ ] 600ms autosave-after-typing-settles for all editable fields, going through the Phase 1 write queue
...
- [ ] Apply meeting template scaffold (see Phase 9) when a template is set at creation time
...
- [ ] Apply filters by clicking card elements (tag/project/team/badge) as described in Phase 3
