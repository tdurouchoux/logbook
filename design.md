...
- `⌘↵` is a global hotkey (`Mod+Enter`), not a listener scoped to the card's own DOM — expanding a card opens its note in Obsidian's editor and usually moves keyboard focus there, so the shortcut has to fire and close/save the currently-expanded card no matter which pane currently has focus.
